...
def _meta_str(meta: Dict[str, Dict], key: str) -> str:
    value = meta.get(key, "")
...
def extract_from_resume(
    ...
    for id, score in ent_scores:
        ...
        results.append(EntityResult(
            id=id_str,
            cosine_score=score,
            entity_type=_meta_str(meta, "entity_type"),
            esco_uri=_meta_str(meta, "esco_uri"),
            label=_meta_str(meta, "preferred_label"),
            code=_meta_str(meta, "code"),
...
def query_type(
    ...
    ent_list = [ents] if isinstance(ents, str) else ents
    for ent in ent_list:
        query_vector = model.encode(
            [ent],
            normalize_embeddings=True,
